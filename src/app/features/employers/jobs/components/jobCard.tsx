...
import { JobCardType } from "../server/jobs.queries"; // imoprt the inferred type
import Image from "next/image";
import { Banknote, Briefcase, Clock, MapPin } from "lucide-react";

interface JobCradProps {
  job: JobCardType;
}
export const JobCard = ({ job }: JobCradProps) => {
  ...
  return (
    <Link
      href={`/jobs/${job.id}`}
      className="group flex flex-col gap-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm transition-all hove:border-blue-500/50 hover:shadow-md"
    >
      {/* Header: Logo & Title */}
